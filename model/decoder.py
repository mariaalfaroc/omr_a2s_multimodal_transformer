# TODO:
# - Hay modulos que se podrían cambiar por los originales de PyTorch como el MHA.
...
class MHA(nn.Module):
    def __init__(self, embedding_dim, num_heads=None, dropout=0, proj_value=True):
        super().__init__()
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory_key, memory_value=None, tgt_mask=None, memory_mask=None, tgt_key_padding_mask=None,
                memory_key_padding_mask=None, predict_n_last_only=None):
...
class DecoderStack(nn.Module):
    def __init__(self, num_dec_layers, d_model, dim_ff):
    ...
    def forward(self, tgt, memory_key, memory_value, tgt_mask, memory_mask, tgt_key_padding_mask, 
                memory_key_padding_mask, use_cache=False, cache=None, predict_last_n_only=False, keep_all_weights=False):
        ...
        for i, dec_layer in enumerate(self.layers):
            output, weights, weights_self = dec_layer(output,
...
class Decoder(nn.Module):
    def __init__(self, d_model, dim_ff, maxlen, out_categories, attention_window=100):
        ...
        self.decoder = DecoderStack(num_dec_layers=8, d_model=d_model, dim_ff=dim_ff)
    ...
    def forward(self, raw_features_1D, enhanced_features_1D, tokens, reduced_size, token_len, features_size, start=0,
                hidden_predict=None, num_pred=None, cache=None, keep_all_weights=False, token_line=None, token_pg=None):
...
class LMLayer(nn.Module):
    ...
    def forward(self, tgt, tgt_mask=None, tgt_key_padding_mask=None, predict_n_last_only=None):
...
class LMStack(nn.Module):
    def __init__(self, num_dec_layers, d_model, dim_ff):
        super(LMStack, self).__init__()
        self.layers = nn.ModuleList([LMLayer(d_model=d_model, dim_ff=dim_ff) for _ in range(num_dec_layers)])

    def forward(self, tgt, tgt_mask, tgt_key_padding_mask, use_cache=False, cache=None, predict_last_n_only=False, keep_all_weights=False):
        ...
        for i, dec_layer in enumerate(self.layers):
            output, weights = dec_layer(output,
                                        tgt_mask=tgt_mask,
                                        tgt_key_padding_mask=tgt_key_padding_mask,
                                        predict_n_last_only=predict_last_n_only)
...
class LMDecoder(nn.Module):
    def __init__(self, d_model, dim_ff, maxlen, out_categories, attention_window=100):
        ...
        self.decoder = LMStack(num_dec_layers=8, d_model=d_model, dim_ff=dim_ff)
    ...
    def forward(self, tokens, token_len, start=0,
                hidden_predict=None, num_pred=None, cache=None, keep_all_weights=False, token_line=None, token_pg=None):
        ...
        output, weights, cache = self.decoder(pos_tokens,
