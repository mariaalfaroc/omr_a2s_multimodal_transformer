...
class krnConverter():
    ...
    def _readSrcFile(self, file_path: str) -> list:
        ...
        for line in in_src:
            if not line.strip().startswith('!'): in_src_nocomments.append(line.split('\t'))
        pass
        ...
        for single_position in positions:
            for it_voice in range(in_score.shape[0]):
                if in_score[it_voice, single_position] == '*':
                    new_element = in_score[it_voice, max(np.where(np.char.startswith(in_score[it_voice], '*clef'))[0])]
    ...
    def _normalizingLengthCleanAndDecoupledSequences(self, in_voices: list) -> list:
        ...
        for it_step in range(len(in_voices[0])):
            ...
            lengths = [len(np.where(np.array(in_voices[it_voice][it_step]) != None)[0]) for it_voice in range(len(in_voices))]
            ...
            if len(set(lengths)) > 1:
                ...
                for it_voice in range(len(in_voices)):
                    if lengths[it_voice] < max_length:
                        in_voices[it_voice][it_step].extend([self.reserved_dot_EncodedCharacter for _ in range(max_length - lengths[it_voice])])
                    pass
                pass                
            pass
        pass
    ...
    def cleanAndDecoupleKernFile(self, file_path: str) -> list:
        ...
        temp_out_score = self._normalizingLengthCleanAndDecoupledSequences(temp_out_score)

        # Removing 'None' symbols and flattening the individual voices:
        out_score = list()
        for it_voice in range(len(temp_out_score)):
            out_voice = [x for xs in temp_out_score[it_voice] for x in xs if x is not None]
            out_score.append(out_voice)
        pass
        ...
        if any([u in in_token for u in self.reserved_words]):               # Relevant reserved tokens
            ...
            if 'rr' in in_token:                                            # Multirest
                ...
                if '[' in in_token:
                    ...
                    if self.keep_ligatures: out_token.append(ligature)
